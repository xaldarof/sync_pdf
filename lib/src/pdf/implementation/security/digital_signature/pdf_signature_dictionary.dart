...
import 'package:dart_crypto_lib/dart_crypto_lib.dart' hide Digest;
...
class PdfSignatureDictionary implements IPdfWrapper {
  ...
  PdfSignatureDictionary(PdfDocument doc, PdfSignature sig) {
    ...
    (PdfDocumentHelper.getHelper(doc).documentSavedList ??= <DocumentSavedHandler>[])
        .add(_documentSaved);
    (PdfDocumentHelper.getHelper(doc).documentSavedListAsync ??= <DocumentSavedHandlerAsync>[])
  ...
  void _addReference() {
    ...
    trans[PdfDictionaryProperties.p] = PdfNumber(
        PdfSignatureHelper.getHelper(_sig!).getCertificateFlagResult(_sig!.documentPermissions));
  ...
  void _addDate() {
    ...
    dictionary!.setProperty(PdfDictionaryProperties.m,
        PdfString("D:${dateFormat.format(dateTime)}+$offsetHours'$offsetMinutes'"));
  }
  ...
  void _documentSaved(Object sender, DocumentSavedArgs e) {
    ...
    _stream!
        .replaceRange(newPos + emptyText.length, newPos + emptyText.length + 1, utf8.encode('>'));
  ...
  Future<void> _documentSavedAsync(Object sender, DocumentSavedArgs e) async {
    ...
    await _saveRangeItemAsync(writer, str, _startPositionByteRange!)
      ...
      await _saveRangeItemAsync(writer, str2, startPosition).then((int startPosition) async {
        await _saveRangeItemAsync(writer, str3, startPosition).then((int startPosition) async {
          await _saveRangeItemAsync(e.writer! as PdfWriter, str4, startPosition)
            ...
            await getPkcs7ContentAsync().then((List<int>? value) async {
              await PdfString.bytesToHexAsync(value!).then((String text) async {
                ...
                _stream!.replaceRange(_firstRangeLength! + 1, newPos, utf8.encode(text));
                final int num3 = (_secondRangeIndex! - newPos) ~/ 2;
                await PdfString.bytesToHexAsync(List<int>.generate(num3, (int i) => 0))
                    .then((String emptyText) async {
                  _stream!.replaceRange(newPos, newPos + emptyText.length, utf8.encode(emptyText));
                  _stream!.replaceRange(
                      newPos + emptyText.length, newPos + emptyText.length + 1, utf8.encode('>'));
                  PdfSecurityHelper.getHelper(_doc.security).encryptor.encrypt = enabled;
                });
              });
            });
  ...
  List<int>? getPkcs7Content() {
    ...
    if (externalSigner != null && PdfSignatureHelper.getHelper(_sig!).externalChain != null) {
      ...
      final List<String> keys =
          PdfCertificateHelper.getPkcsCertificate(_cert!).getContentTable().keys.toList();
      bool isContinue = true;
      // ignore: avoid_function_literals_in_foreach_calls
      keys.forEach((String key) {
        if (isContinue &&
            PdfCertificateHelper.getPkcsCertificate(_cert!).isKey(key) &&
            PdfCertificateHelper.getPkcsCertificate(_cert!).getKey(key)!.key!.isPrivate!) {
      ...
      final List<X509Certificates> certificates =
          PdfCertificateHelper.getPkcsCertificate(_cert!).getCertificateChain(certificateAlias)!;
      // ignore: avoid_function_literals_in_foreach_calls
    ...
    final List<int>? sh = pkcs7
        .getSequenceDataSet(hash, ocspByte, crlBytes, _sig!.cryptographicStandard)
        .getEncoded(Asn1.der);
    ...
    pkcs7.setSignedData(extSignature!, null, externalSignature.getEncryptionAlgorithm());
    return pkcs7.sign(
        hash, null, null, ocspByte, crlBytes, _sig!.cryptographicStandard, hasalgorithm);
  }
  ...
  Future<List<int>?> getPkcs7ContentAsync() async {
    ...
    List<X509Certificate?>? chain = <X509Certificate?>[];
    final IPdfExternalSigner? externalSigner = PdfSignatureHelper.getHelper(_sig!).externalSigner;
    //check external signer
    if (externalSigner != null && PdfSignatureHelper.getHelper(_sig!).externalChain != null) {
      chain = PdfSignatureHelper.getHelper(_sig!).externalChain;
      final String digest = getDigestAlgorithm(externalSigner.hashAlgorithm);
      final _SignaturePrivateKey pks = _SignaturePrivateKey(digest);
      hashAlgoritm = pks.getHashAlgorithm();
      externalSignature = pks;
    } else {
      String certificateAlias = '';
      await PdfCertificateHelper.getPkcsCertificate(_cert!)
        ...
        final KeyEntry pk =
            PdfCertificateHelper.getPkcsCertificate(_cert!).getKey(certificateAlias)!;
        await PdfCertificateHelper.getPkcsCertificate(_cert!)
    ...
    final _PdfCmsSigner pkcs7 = _PdfCmsSigner(null, chain!, hashAlgoritm!, false);
    final IRandom source = getUnderlyingSource();
    final List<IRandom?> sources = List<IRandom?>.generate(_range.length ~/ 2, (int i) => null);
    for (int j = 0; j < _range.length; j += 2) {
      sources[j ~/ 2] = _WindowRandom(source, _range[j], _range[j + 1]);
    ...
    final PdfStreamReader data = _RandomStream(_RandomGroup(sources));
    await pkcs7._digestAlgorithm.digestAsync(data, hashAlgoritm).then((List<int>? hash) async {
      await pkcs7
          .getSequenceDataSetAsync(hash!, ocspByte, crlBytes, _sig!.cryptographicStandard)
          .then((DerSet derSet) async {
        await derSet.getEncodedAsync(Asn1.der).then((List<int>? sh) async {
          List<int>? extSignature;
          if (externalSigner != null) {
            await externalSigner.sign(sh!).then((SignerResult? signerResult) async {
              ...
              if (extSignature != null) {
                ...
                pkcs7Content = List<int>.filled(_estimatedSize, 0, growable: true);
              }
            });
          } else {
          ...
          if (pkcs7Content == null) {
            ...
            pkcs7Content = await pkcs7.signAsync(
                hash, null, null, ocspByte, crlBytes, _sig!.cryptographicStandard, hashAlgoritm);
          }
        });
      });
    });
  ...
  String getDigestAlgorithm(DigestAlgorithm? digest) {
    ...
    switch (digest) {
      ...
      case DigestAlgorithm.sha512:
        ...
        break;
      case DigestAlgorithm.gost1106:
        digestAlgorithm = MessageDigestAlgorithms.secureGost1106;
        break;
  ...
  Future<int> _saveRangeItemAsync(PdfWriter writer, String str, int startPosition) async {
    final List<int> date = utf8.encode(str);
    writer.buffer!.replaceRange(startPosition, startPosition + date.length, date);
    return startPosition + str.length;
  }
...
class MessageDigestAlgorithms {
  ...
  MessageDigestAlgorithms() {
    ...
    _digests['GOST1106'] = '1.1.1.1.1.1';
    _algorithms = <String?, String>{};
    _algorithms['GOST1106'] = 'GOST1106';
  ...
  static const String secureGost1106 = 'GOST1106';
  ...
  /// internal method
  Future<List<int>?> digestAsync(PdfStreamReader data, dynamic hashAlgorithm) async {
    dynamic algorithm;
    algorithm =
        hashAlgorithm is String ? await getMessageDigestAsync(hashAlgorithm) : hashAlgorithm;

    if (hashAlgorithm == 'gost1106') {
      return CryptoDart.calculateHashOzdst1106(data.data);
    } else {
      final dynamic output = AccumulatorSink<Digest>();
      final dynamic input = algorithm.startChunkedConversion(output);
      int? count;
...
class _PdfCmsSigner {
  ...
  //Implementation
  DerSet getSequenceDataSet(List<int> secondDigest, List<int>? ocsp, List<List<int>>? crlBytes,
    ...
    if (sigtype == CryptographicStandard.cades) {
      ...
      v.encodableObjects.add(DerSet(array: <Asn1Encode>[
        DerSequence.fromObject(DerSequence.fromObject(DerSequence(collection: aaV2)))
      ]));
      attribute.encodableObjects.add(DerSequence(collection: v));
    }
  ...
  //Implementation
  Future<DerSet> getSequenceDataSetAsync(List<int> secondDigest, List<int>? ocsp,
      List<List<int>>? crlBytes, CryptographicStandard? sigtype) async {
    final Asn1EncodeCollection attribute = Asn1EncodeCollection();
    Asn1EncodeCollection v = Asn1EncodeCollection();
    v.encodableObjects.add(DerObjectID(_DigitalIdentifiers.contentType));
    v.encodableObjects.add(DerSet(array: <Asn1Encode>[DerObjectID(_DigitalIdentifiers.pkcs7Data)]));
      ...
      await alg
        ...
        await alg.getMessageDigestAsync(hashAlgorithm!).then((dynamic value) {
          aaV2.encodableObjects
              .add(DerOctet(value.convert(_signCert!.c!.getEncoded(Asn1.der)).bytes as List<int>));
          v.encodableObjects.add(DerSet(array: <Asn1Encode>[
            DerSequence.fromObject(DerSequence.fromObject(DerSequence(collection: aaV2)))
          ]));
          attribute.encodableObjects.add(DerSequence(collection: v));
  ...
  Future<void> setSignedDataAsync(
      List<int> digest, List<int>? rsaData, String? digestEncryptionAlgorithm) async {
    _signedData = digest;
    _signedRsaData = rsaData;
    if (digestEncryptionAlgorithm != null) {
      if (digestEncryptionAlgorithm == 'RSA') {
  ...
  List<int>? sign(
    ...
    _certificates.forEach((X509Certificate? xcert) {
      v.encodableObjects
          .add(Asn1Stream(PdfStreamReader(xcert!.c!.getEncoded(Asn1.der))).readAsn1());
    });
    ...
    signerinfo.encodableObjects
        .add(DerTag(0, getSequenceDataSet(secondDigest, ocsp, crls, sigtype), false));
    v = Asn1EncodeCollection();
...
/// internal type definition
typedef DocumentSavedHandlerAsync = Future<void> Function(Object sender, DocumentSavedArgs args);
